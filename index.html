...
// Simulación de importaciones de Firebase - en un proyecto real serían 'firebase/app', etc.
// No se necesita una importación real aquí ya que usaremos los scripts en el HTML imaginario.
...
// --- DATOS INICIALES (SOLO PARA LA PRIMERA CARGA A LA BASE DE DATOS) ---
    ...
    <>
        <style>{`
            ...
            .ember {
                ...
                background: radial-gradient(ellipse at center, rgba(255, 100, 0, 0.6) 0%, rgba(255, 100, 0, 0) 70%);
...
const CustomerMenu = ({ menuData, onNavigate }) => {
  ...
  if (!menuData) {
    return <div className="text-white text-center p-10">Cargando menú...</div>;
  }
  
  // --- INSTRUCCIÓN PARA IMPORTAR LA FUENTE ---
  // Para que 'Barista Script' funcione, añade esta línea en el <head> de tu archivo HTML:
  // <link href="https://fonts.cdnfonts.com/css/barista-script" rel="stylesheet">
                
  const currentCategory = menuData.categories.find(c => c.id === activeCategory);
  const commonTabClass = "px-3 py-2 text-lg font-normal rounded-full transition-all duration-300 cursor-pointer whitespace-nowrap bg-black/30 backdrop-blur-sm border border-white/10";
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-gray-900 font-sans_alt overflow-hidden rounded-3xl shadow-2xl shadow-black/50 border border-gray-700 relative">
      <BonfireEffect />
      <div className="relative z-10">
        <header className="p-6 text-center">
          ...
          <h1 className="text-6xl font-normal text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-yellow-100 to-yellow-300" style={{fontFamily: "'Barista Script', cursive", textShadow: '2px 2px 4px rgba(0,0,0,0.5)'}}>{menuData.restaurantName}</h1>
        </header>
        <nav className="p-4 overflow-x-auto scrollbar-hide">
             <style>{`.scrollbar-hide::-webkit-scrollbar { display: none; } .scrollbar-hide { -ms-overflow-style: none; scrollbar-width: none; }`}</style>
            <div className="flex justify-center items-center space-x-2 sm:space-x-4">
                {menuData.categories.map(category => (
                    <button key={category.id} onClick={() => setActiveCategory(category.id)} style={{fontFamily: "'Barista Script', cursive"}} className={`${commonTabClass} ${activeCategory === category.id ? activeTabClass : inactiveTabClass}`}>{category.name}</button>
                ))}
            </div>
        </nav>
        <main className="p-4 sm:p-5">{currentCategory && (<div className="space-y-6">{currentCategory.items.map(item => (
            <div key={item.id} className="transition-all duration-300 hover:scale-[1.03] rounded-2xl overflow-hidden">
                ...
                <div className="p-5 bg-black/30 backdrop-blur-sm border-t border-white/10 rounded-b-2xl" style={{textShadow: '1px 1px 3px rgba(0,0,0,0.7)'}}>
                    <h3 className="text-3xl font-normal text-white mb-2" style={{fontFamily: "'Barista Script', cursive"}}>{item.name}</h3>
...
const AdminDashboard = ({ menuData, onUpdate, onLogout }) => {
  ...
  const handleItemChange = (categoryId, itemId, field, value) => {
    const updatedData = JSON.parse(JSON.stringify(localMenuData));
  ...
  const handleImageUpload = (e, categoryId, itemId) => {
    const file = e.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        handleItemChange(categoryId, itemId, 'imageUrl', reader.result);
      };
  ...
  const handleAddItem = (categoryId) => {
    const newItem = {
        ...
        price: 0.00,
        imageUrl: "https://placehold.co/600x400/cccccc/FFFFFF?text=Subir+Imagen"
    };
    const updatedData = JSON.parse(JSON.stringify(localMenuData));
    const category = updatedData.categories.find(c => c.id === categoryId);
  ...
  const handleSaveChanges = () => {
    onUpdate(localMenuData);
    alert("¡Cambios guardados en la base de datos!");
  };

  if (!localMenuData) {
    return <div className="text-white text-center p-10">Cargando datos...</div>;
  }
    ...
    <div className="w-full max-w-4xl mx-auto bg-gray-800 text-white rounded-2xl shadow-lg p-6 sm:p-8 font-sans">
       <header className="flex justify-between items-center mb-8">
        <div>
            ...
            <p className="text-lg text-gray-400">{localMenuData.restaurantName}</p>
        </div>
        <div>
            <button onClick={handleSaveChanges} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg mr-4">Guardar Cambios</button>
            <button onClick={onLogout} className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg"><LogoutIcon/></button>
        </div>
      </header>
      <main>
            ...
            <div className="w-full max-w-md p-8 space-y-8 bg-gray-800 rounded-2xl shadow-lg">
                ...
                <div className="space-y-6">
                    <div>
                        ...
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-3 mt-2 text-white bg-gray-700 rounded-md outline-none focus:ring-2 focus:ring-orange-500"/>
                    </div>
                    <div>
                        <label className="text-sm font-bold text-gray-400">Contraseña</label>
                        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full p-3 mt-2 text-white bg-gray-700 rounded-md outline-none focus:ring-2 focus:ring-orange-500"/>
                    </div>
...
export default function App() {
    // Simulación de Firebase y enrutamiento
    const [route, setRoute] = useState('/');
    const [user, setUser] = useState(null); // Simula el usuario autenticado
    const [authError, setAuthError] = useState('');
    const [menuData, setMenuData] = useState(null);

    // Simulación de la base de datos local
    const db = useRef({ menu: null });

    // Cargar datos iniciales en la 'base de datos'
    useEffect(() => {
        if (!db.current.menu) {
            console.log("Cargando datos iniciales en la base de datos simulada...");
            db.current.menu = initialMenuData;
            setMenuData(initialMenuData);
        } else {
            setMenuData(db.current.menu);
        }
    }, []);
    ...
    const handleRegister = (email, password) => {
        if (email && password) {
            ...
            setAuthError('Por favor, ingrese correo y contraseña para registrar.');
        }
    };
    ...
    const handleUpdateMenu = (newData) => {
        console.log("Guardando datos en la base de datos simulada...");
        db.current.menu = newData;
        setMenuData(newData);
    };


    // --- RENDERIZADO CONDICIONAL BASADO EN LA RUTA Y AUTENTICACIÓN ---
    if (route === '/admin') {
        if (user) {
            return <AdminDashboard menuData={menuData} onUpdate={handleUpdateMenu} onLogout={handleLogout} />;
        } else {
